...
class MapDrawer {
    ...
    async draw() {
        ...
        let mapCanvas = Canvas.createCanvas(scaledCanvasWidth, scaledCanvasHeight);
        ...
        if (this.settings.crop_left !== 0 || this.settings.crop_top !== 0 || this.settings.crop_bottom !== 0 || this.settings.crop_right !== 0) {
            ...
            try {
                ...
                return await this.rotateMap(this.settings, croppedMapCanvas);
            } catch (error) {
                Logger.error("Unable to crop and pad canvas", error)
            }            
        }

        return await this.rotateMap(this.settings, mapCanvas);
    }

    async rotateMap(settings, mapCanvas) {
        const Jimp = require('jimp');

        const image = await Jimp.read(mapCanvas.toBuffer("image/png"));
        
        image.rotate(settings.rotate);

        const rawImg = await image.getBufferAsync(Jimp.MIME_PNG);
